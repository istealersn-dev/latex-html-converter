...
class AssetValidationError(BaseServiceError):
    ...
    def __init__(self, message: str, asset_file: str, details: Dict[str, Any] | None = None):
...
class AssetValidator:
    ...
    def validate_svg(
        ...
        try:
            ...
            raise AssetValidationError(f"Unexpected SVG validation error: {exc}", str(svg_file))

    def _validate_svg_file(self, svg_file: Path) -> None:
        """Validate the SVG input file."""
        if not svg_file.exists():
            raise AssetValidationFileError(f"SVG file not found: {svg_file}", str(svg_file))

        if not svg_file.is_file():
            raise AssetValidationFileError(f"SVG path is not a file: {svg_file}", str(svg_file))
    ...
    def _read_svg_content(self, svg_file: Path) -> str:
        ...
        try:
            ...
            raise AssetValidationFileError(f"Cannot read SVG file: {exc}", str(svg_file))
    ...
    def _validate_svg_structure(self, content: str) -> Dict[str, Any]:
        ...
        if not has_viewbox and not has_dimensions:
            results["structure_warnings"].append("Missing viewBox or width/height attributes")
    ...
    def _validate_svg_quality(self, content: str, svg_file: Path) -> Dict[str, Any]:
        ...
        if '<!--' in content:
            results["quality_warnings"].append("Contains comments that could be removed")

        if re.search(r'\s{2,}', content):
            results["quality_warnings"].append("Contains excessive whitespace")

        if '<?xml' in content:
            results["quality_warnings"].append("Contains XML declaration (may be unnecessary)")
        ...
        if path_count > 100:
            results["quality_warnings"].append("High number of paths may impact rendering")
    ...
    def _validate_svg_accessibility(self, content: str) -> Dict[str, Any]:
        ...
        if not has_title:
            results["accessibility_warnings"].append("Missing title element for accessibility")

        # Check for desc element
        has_desc = '<desc>' in content
        results["accessibility_metrics"]["has_desc"] = has_desc
        if not has_desc:
            results["accessibility_warnings"].append("Missing description element for accessibility")

        # Check for alt text on images
        images_without_alt = re.findall(r'<image[^>]*(?!.*alt=)[^>]*>', content)
        if images_without_alt:
            results["accessibility_warnings"].append(f"Images without alt text: {len(images_without_alt)}")
    ...
    def get_validation_summary(self, validation_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        avg_quality_score = sum(r.get("quality_score", 0) for r in validation_results) / total_files if total_files > 0 else 0
