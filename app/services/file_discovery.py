...
class FileDiscoveryService:
    ...
    def __init__(self):
        ...
        self.latex_extensions = {'.tex', '.cls', '.sty', '.bib', '.bst', '.bbl', '.aux', '.toc', '.lof', '.lot'}
        self.graphics_extensions = {'.pdf', '.png', '.jpg', '.jpeg', '.eps', '.ps', '.svg'}
        ...
        self.graphics_pattern = re.compile(r'\\includegraphics(?:\[[^\]]*\])?\{([^}]+)\}')
    ...
    def discover_latex_files(self, zip_path: Path) -> ProjectStructure:
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as zip_file:
                ...
                all_files = [Path(f) for f in zip_file.namelist() if not f.endswith('/')]
                ...
                self.logger.info(f"Discovered {len(all_files)} files, main file: {main_tex}")
                return project_structure
                
        except zipfile.BadZipFile as e:
            raise ValueError(f"Invalid ZIP file: {e}")
        except Exception as e:
            raise ValueError(f"Error discovering files: {e}")

    def extract_project_files(self, zip_path: Path, output_dir: Path) -> ProjectStructure:
        """
        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as zip_file:
                ...
                project_structure.main_tex_file = output_dir / project_structure.main_tex_file
                project_structure.project_dir = output_dir
                
                self.logger.info(f"Extracted {len(project_structure.extracted_files)} files to {output_dir}")
                return project_structure
                
        except Exception as e:
            raise ValueError(f"Error extracting files: {e}")

    def _find_main_tex_file(self, files: List[Path]) -> Optional[Path]:
        """
        ...
        main_candidates = ['main.tex', 'document.tex', 'paper.tex', 'article.tex', 'manuscript.tex']
    ...
    def _parse_tex_dependencies(self, zip_file: zipfile.ZipFile, main_tex: Path) -> LatexDependencies:
        """
        ...
        try:
            ...
            self.logger.info(f"Parsed dependencies: {len(dependencies.packages)} packages, "
    ...
    def find_missing_files(self, project_structure: ProjectStructure) -> List[str]:
        ...
        for input_file in dependencies.input_files:
            if not any(f.name == input_file or f.name == f"{input_file}.tex" 
        ...
        for custom_class in dependencies.custom_classes:
            if not any(f.name == custom_class or f.name == f"{custom_class}.cls" 
                      for f in project_structure.supporting_files.get('class_files', [])):
                missing_files.append(custom_class)
        
        return missing_files

    def validate_project_structure(self, project_structure: ProjectStructure) -> Dict[str, Any]:
        """
        ...
        if not project_structure.main_tex_file.exists():
            ...
            diagnostics['errors'].append(f"Main file not found: {project_structure.main_tex_file}")
        ...
        if missing_files:
            ...
            diagnostics['suggestions'].append("Ensure all referenced files are included in the ZIP")
        
        # Check for custom document classes
        if project_structure.dependencies.custom_classes:
            diagnostics['suggestions'].append(
                f"Custom document classes found: {project_structure.dependencies.custom_classes}. "
        ...
        if len(project_structure.dependencies.packages) > 20:
            diagnostics['warnings'].append(
                f"Many packages required ({len(project_structure.dependencies.packages)}). "
