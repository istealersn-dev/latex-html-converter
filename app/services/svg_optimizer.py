...
class SVGOptimizationError(BaseServiceError):
    ...
    def __init__(
        self, message: str, svg_file: str, details: dict[str, Any] | None = None
    ):
...
class SVGOptimizer:
    ...
    def optimize_svg(
        ...
        try:
            ...
            logger.info(
                f"SVG optimization successful: {compression_ratio:.2%} size reduction"
            )
            return result
            ...
            raise SVGOptimizationError(
                f"Unexpected SVG optimization error: {exc}", str(svg_file)
            ) from exc
    ...
    def _validate_svg_file(self, svg_file: Path) -> None:
        ...
        try:
            ...
            if not self._is_valid_svg(content):
                raise SVGOptimizationFileError(
                    f"Invalid SVG file: {svg_file}", str(svg_file)
                )
        except Exception as exc:
            raise SVGOptimizationFileError(
                f"Cannot validate SVG file: {exc}", str(svg_file)
            ) from exc
    ...
    def _write_svg_content(self, svg_file: Path, content: str) -> None:
        ...
        try:
            ...
            raise SVGOptimizationFileError(
                f"Cannot write SVG file: {exc}", str(svg_file)
            ) from exc
    ...
    def _is_valid_svg(self, content: str) -> bool:
        ...
        return (
            ...
            or content_lower.startswith("<svg")
            or "<svg" in content_lower
        )
    ...
    def _optimize_paths(self, content: str) -> str:
        ...
        def optimize_path(match):
            ...
            optimized_path = re.sub(
                r"([MLHVCSQTAZ])\s*([MLHVCSQTAZ])", r"\1", path_data
            )
    ...
    def batch_optimize_svg(
        ...
        for svg_file in svg_files:
            try:
                ...
                results.append(
                    {"success": False, "error": str(exc), "svg_file": str(svg_file)}
                )
    ...
    def get_optimization_info(self, svg_file: Path) -> dict[str, Any]:
        ...
        try:
            ...
            analysis = {
                ...
                "estimated_optimization_ratio": self._estimate_optimization_ratio(
                    content
                ),
            }
            ...
            return {"file_path": str(svg_file), "error": str(exc)}
