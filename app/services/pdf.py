...
class PDFConversionError(BaseServiceError):
    ...
    def __init__(self, message: str, pdf_file: str, details: Dict[str, Any] | None = None):
...
class PDFConversionService:
    ...
    def _verify_tool_installations(self) -> None:
        ...
        for tool_path, tool_name in tools:
            try:
                ...
                raise PDFConversionError(
                    f"{tool_name} not found or not working: {exc}",
                    "system"
                )
    ...
    def convert_pdf_to_svg(
        ...
        try:
            ...
            if pdf_info.get("pages", 1) == 1 and pdf_info.get("has_vector_content", True):
                # Use ghostscript for vector PDFs
                svg_file = self._convert_pdf_with_ghostscript(pdf_file, output_dir, options)
            else:
                # Use pdftoppm + conversion for complex PDFs
                svg_file = self._convert_pdf_with_pdftoppm(pdf_file, output_dir, options)
            ...
            result = {
                ...
                "optimization_ratio": self._calculate_optimization_ratio(pdf_file, optimized_svg),
            ...
            raise PDFConversionError(f"Unexpected PDF conversion error: {exc}", str(pdf_file))

    def _validate_pdf_file(self, pdf_file: Path) -> None:
        """Validate the PDF input file."""
        if not pdf_file.exists():
            raise PDFConversionFileError(f"PDF file not found: {pdf_file}", str(pdf_file))

        if not pdf_file.is_file():
            raise PDFConversionFileError(f"PDF path is not a file: {pdf_file}", str(pdf_file))
        ...
        try:
            ...
            if result.returncode != 0:
                raise PDFConversionFileError(
                    f"Invalid PDF file: {result.stderr}",
                    str(pdf_file)
                )
        except Exception as exc:
            raise PDFConversionFileError(f"Cannot validate PDF file: {exc}", str(pdf_file))
    ...
    def _get_pdf_info(self, pdf_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode != 0:
                raise PDFConversionError(
                    f"Failed to get PDF info: {result.stderr}",
                    str(pdf_file)
                )
    ...
    def _convert_pdf_with_ghostscript(
        ...
        try:
            ...
            if not svg_file.exists():
                raise PDFConversionError(
                    "SVG file not created after ghostscript conversion",
                    str(pdf_file)
                )
            ...
            if isinstance(exc, PDFConversionError):
                raise
            raise PDFConversionError(f"Ghostscript conversion failed: {exc}", str(pdf_file))
    ...
    def _convert_pdf_with_pdftoppm(
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if not svg_file.exists():
                    raise PDFConversionError(
                        "SVG file not created after image conversion",
                        str(pdf_file)
                    )
            ...
            if isinstance(exc, PDFConversionError):
                raise
            raise PDFConversionError(f"pdftoppm conversion failed: {exc}", str(pdf_file))
